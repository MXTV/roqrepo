...
if quality == '13':
	...
	xbmc.log( 'record.py: osplatform= %s' % osplatform)
	try:
		import shutil
	...
	try:
		...
		xbmc.log( 'record.py: try to copy %s --> %s' % (osplatformexe,rtmpdumpEXEp))
		shutil.copyfile(os.path.join(ADDON.getAddonInfo('path'),'rtmpdump',osplatformexe),rtmpdumpEXEp) # Folder should xist!	
		xbmc.log( 'record.py: copied %s --> %s' % (osplatformexe,rtmpdumpEXEp))
	except:
		pass
		xbmc.log( 'record.py: copied FAILED %s --> %s' % (osplatformexe,rtmpdumpEXEp))
	try:
		os.chmod(rtmpdumpEXEp, 0777) 
	except:
		pass
		xbmc.log( 'record.py: set 0777 FAILED %s --> /system/bin/rtmpdump' % osplatformexe)

	rtmpdumpEXE = rtmpdumpEXEp
else:
	rtmpdumpEXE = os.path.join(ADDON.getAddonInfo('path'),'rtmpdump',rtmpdumpEXEp)
xbmc.log( 'record.py: rtmpdumpEXE= %s' % repr(rtmpdumpEXE))
xbmc.log( 'record.py: os.path.exists(rtmpdumpEXE)= %s' % repr(os.path.exists(rtmpdumpEXE)))
...
if not xbmc.getCondVisibility('system.platform.windows'):
	if os.access(rtmpdumpEXE, os.X_OK):
		xbmc.log( 'Permissions ------ 0777 ----- GREAT !!' ) # Put in LOG
	else:
		xbmc.log( 'Permissions -----------------   BAD !!' ) # Put in LOG
		for dirpath, dirnames, filenames in os.walk(os.path.join(ADDON.getAddonInfo('path'),'rtmpdump')):
			for filename in filenames:
				path = os.path.join(dirpath, filename)
				try:
					os.chmod(path, 0777) 
					xbmc.log( 'Permissions set with: CHMOD 0777 !!' ) # Put in LOG
				except: pass
# LINUX ?
try:
