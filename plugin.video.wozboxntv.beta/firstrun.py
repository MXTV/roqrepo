...
def Launch():        
	dialog = xbmcgui.Dialog()
	if dialog.yesno("WOZBOX NTV", 'Do you Wish To Register','', "Or Sign In",'Register','Sign In'):
	
		SIGNIN()
	
	else:
		if recordings.referralLink() == '1':
			dialog.ok("WOZBOX NTV", 'Please register for WOZBOX NTV at wozboxtv.com/registration','After registering check your email to verify account','Then come back and sign in.',)
		else:
			...
			url='http://www.ntv.mx/index.php?' + recordings.referral()+ 'c=1&a=1&xbmc=1&r=xbmc&accdata={"email":"%s","firstname":"%s","lastname":"%s","psw":"%s","cntid":"%s","phnnm":"%s"}'%(email,firstname,surname,password,country_id,phone)
			link=OPEN_URL(url)
			if 'success' in link:
				dialog.ok("WOZBOX NTV", 'Thank You For Registering','Please Open Your Email Client', 'And Verify Your Email Address')
				ADDON.setSetting('user',email)
				ADDON.setSetting('pass',password)
				ADDON.setSetting('firstrun','true')
			else:
				winput=re.compile('"message":"(.+?)"').findall(link)
				if dialog.yesno("WOZBOX NTV ", winput[0],'', 'Exit And Restart Again'):
					Launch()
				else:
					EXIT()
