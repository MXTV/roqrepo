...
def LOGIN():
        loginurl = 'http://www.ntv.mx/index.php?' + recordings.referral() + 'c=3&a=4'
        ...
        headers  = {'Host':'www.ntv.mx',
                                                ...
                                                'Referer':'http://www.ntv.mx/index.php?' + recordings.referral() + 'c=3&a=0','User-Agent' : UA}
...
def AUTH():
        ...
        username    =ADDON.getSetting('user')
        password =md5(ADDON.getSetting('pass')).hexdigest()
        url = 'http://ntv.mx/?' + recordings.referral() + 'c=3&a=4&email=%s&psw2=%s&rmbme=on'%(username,password)
...
def SUBS():
    net.set_cookies(cookie_jar)
    link = net.http_GET('http://www.ntv.mx/?' + recordings.referral() + 'c=1&a=18', headers={'User-Agent' : UA}).content
    data = json.loads(link)
    body = data['body']
    for field in body:
        title= field['title']
...
def ORDERS():
    net.set_cookies(cookie_jar)
    link = net.http_GET('http://www.ntv.mx/?' + recordings.referral() + 'c=1&a=19', headers={'User-Agent' : UA}).content.encode('ascii', 'ignore')
    data = json.loads(link)
    body = data['body']
    for field in body:
...
def BUYSUBS():
    net.set_cookies(cookie_jar)
    link = net.http_GET('http://ntv.mx/?' + recordings.referral() + 'c=4&a=7', headers={'User-Agent' : UA}).content
    data = json.loads(link)
    for field in data:
        cat= str(field['id'])
...
def CARDDETAILS(cat):
		...
		if not 'Cancel' in type:
			...
			url='https://www.ntv.mx/?' + recordings.referral() + 'c=8&a=15&oid=%s&card={"type":"%s","number":"%s","name":"%s","month":"%s","year":"%s","cvv":"%s"}'%(cat,type,card,name,month,year,cvv)
			net.set_cookies(cookie_jar)
...
class card_payment(xbmcgui.WindowXMLDialog):
    ...
    def onClick(self, controlID):
        if controlID == 7:
            ...
            net.set_cookies(cookie_jar)
            url = 'https://www.ntv.mx/?' + recordings.referral() + 'c=8&a=15&oid=%s&card={"type":"%s","number":"%s","name":"%s","month":"%s","year":"%s","cvv":"%s"}'%(self.cat,
                                                                                                                                                 self.card_type,
                                                                                                                                                 self.card_number,
                                                                                                                                                 self.name,
                                                                                                                                                 self.expiry_month,
...
def PAYSUBS(cat):
	dialog = xbmcgui.Dialog()
	net.set_cookies(cookie_jar)
	URL='http://www.ntv.mx/?' + recordings.referral() + 'c=4&a=8&item_id=%s&amount=1'%cat
	link = net.http_GET(URL, headers={'User-Agent' : UA}).content
	data = json.loads(link)
	if 'success' in link:
		cat= str(data['body'])
...
def RecordingsPlanned():
	...
	#print 'RecordingsPlanned recordingsD= %s' % repr(recordingsD)
...
def showRecording(recordingsC,index):
		import recordings
		now = recordings.parseDate(datetime.datetime.today().strftime('%Y-%m-%d %H:%M:%S'))
	...
	#for index in range(0, len(recordingsC)): 
		...
		endDateTest =  endDate + datetime.timedelta(days = 2)
		...
		if (not ignoreRecording and ((recordings.parseDate(endDateTest) > recordings.parseDate(datetime.datetime.today().strftime('%Y-%m-%d'))) or ('Recursive:' in name))):
...
def PLAY_STREAM(name,url,iconimage,cat):
	...
	startD = recordings.parseDate(datetime.datetime.now())
	endD = startD
	if PlayDefaultStream:
		recordingsActive = ""
	else:
		recordingsActive = recordings.getRecordingsActive(startD,endD)
	if (recordingsActive != "") and ('Stop Recording' not in recordingsActive) and locking.isAnyRecordLocked():
	...
	if '"allown":false' in link:
		try:
			...
			dialog = xbmcgui.Dialog()
			dialog.ok('WOZBOX NTV', '', match[0].replace('\/','/'))
		except:
			dialog = xbmcgui.Dialog()
			dialog.ok('WOZBOX NTV', '', 'Please Sign Up To Watch The Streams')
	else:
...
def addDir(name,url,mode,iconimage,cat,date,description,startDate='',endDate='',recordname=''):
		u=sys.argv[0]+"?url="+urllib.quote_plus(url)+"&mode="+str(mode)+"&name="+urllib.quote_plus(name)+"&iconimage="+urllib.quote_plus(iconimage)+"&cat="+urllib.quote_plus(cat)+"&date="+str(date)+"&description="+urllib.quote_plus(str(description))+"&startDate="+str(startDate)+"&endDate="+str(endDate)+"&recordname="+urllib.quote_plus(recordname)
		ok=True
		...
		if mode==200 or mode==12:
			...
			if ADDON.getSetting('enable_record')=='true':
				if not name[0:9] =='Recursive':
					menu.append(('[COLOR red][B]RECORD[/B][/COLOR]','XBMC.RunPlugin(%s?mode=2001&url=url&cat=%s&startDate=%s&endDate=%s&recordname=%s&description=%s)'% (sys.argv[0],urllib.quote_plus(cat),startDate,endDate,urllib.quote_plus(recordname),urllib.quote_plus(str(description)))))
					menu.append(('[COLOR red][B]RECORD RECURSIVE[/B][/COLOR]','XBMC.RunPlugin(%s?mode=2005&url=url&cat=%s&startDate=%s&endDate=%s&recordname=%s&description=%s)'% (sys.argv[0],urllib.quote_plus(cat),startDate,endDate,urllib.quote_plus(recordname),urllib.quote_plus(str(description)))))
				if view == '3':
					menu.append(('[COLOR red][B]MODIFY RECORD[/B][/COLOR]','XBMC.RunPlugin(%s?mode=2003&url=url&cat=%s&startDate=%s&endDate=%s&recordname=%s)'% (sys.argv[0],urllib.quote_plus(cat),startDate,endDate,urllib.quote_plus(recordname))))
					menu.append(('[COLOR orange][B]DISABLE RECORD[/B][/COLOR]','XBMC.RunPlugin(%s?mode=2006&url=url&cat=%s&startDate=%s&endDate=%s&recordname=%s&description=%s)'% (sys.argv[0],urllib.quote_plus(cat),startDate,endDate,urllib.quote_plus(recordname),urllib.quote_plus(str(description)))))
					menu.append(('[COLOR red][B]DELETE RECORD[/B][/COLOR]','XBMC.RunPlugin(%s?mode=2002&url=url&cat=%s&startDate=%s&endDate=%s&recordname=%s)'% (sys.argv[0],urllib.quote_plus(cat),startDate,endDate,urllib.quote_plus(recordname))))
				if not view == '3':
			...
			try:
				if name[0:9] =='Recursive' and not locking.isAnyRecordLocked():
					menu.append(('[COLOR orange][B]Refresh Recursive[/B][/COLOR]','XBMC.RunPlugin(%s?name=None&mode=2007&url=None&iconimage=None&cat=%s&startDate=%s&endDate=%s&recordname=%s&description=%s)'% (sys.argv[0],urllib.quote_plus(cat),startDate,endDate,urllib.quote_plus(recordname),urllib.quote_plus(str(description)))))
			except:
				pass
			menu.append(('[COLOR orange][B]Refresh[/B][/COLOR]','XBMC.RunPlugin(%s?name=None&mode=2004&url=None&iconimage=None&cat=%s&startDate=%s&endDate=%s&recordname=%s&description=%s)'% (sys.argv[0],urllib.quote_plus(cat),startDate,endDate,urllib.quote_plus(recordname),urllib.quote_plus(str(description)))))
...
params=get_params()
print 'wozboxntv default.py: params= %s' % str(repr(params))
url=None
name=None
mode=None
iconimage=None
...
if mode==None or url==None or len(url)<1:
	...
	if ADDON.getSetting('RecordFromTVguide')=='true':
		...
		startDate =  (datetime.datetime.today() + datetime.timedelta(minutes=1)).strftime('%Y-%m-%d %H:%M:%S')
		endDate = (datetime.datetime.today() + datetime.timedelta(minutes=recordduration)).strftime('%Y-%m-%d %H:%M:%S')
