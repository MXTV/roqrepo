...
SERVER_ORDER = {'tvaddons': 3, 'pastebin': 1, 'dropbox': 4, 'ubuntu': 2}
        ...
        for _, name in FILES:
            for service in results:
                if name in results[service]['results']:
                    ...
                    args.append(line)
                    log_utils.log('[COLOR white]Log Uploaded: %s: %s[/COLOR]' % (name, results[service]['results'][name]), log_utils.LOGNOTICE)
...
def __confirm_upload():
    return xbmcgui.Dialog().yesno('[COLOR white]Fab Hosting[/COLOR]', ' ', '[COLOR white]Are you sure you want to upload your logs?[/COLOR]', ' ')
