...
def TxtBox(announce):
	class TextBox():
		...
		def setControls(self):
			self.win.getControl(self.CONTROL_LABEL).setLabel('[COLOR white]Important Disclaimer[/COLOR]') # set heading
...
def TextBoxesPlain(announce):
	class TextBox():
		...
		def __init__(self,*args,**kwargs):
			...
			self.setControls()
		def setControls(self):
			self.win.getControl(self.CONTROL_LABEL).setLabel('[COLOR white]Fab Log Viewer[/COLOR]') # set heading
			try: f=open(announce); text=f.read()
			except: text=announce
			self.win.getControl(self.CONTROL_TEXTBOX).setText(str(text))
			return
