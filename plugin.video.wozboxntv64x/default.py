...
if ADDON.getSetting('oldpass')=='':                                   ### NTV.mx 3.5.0
    ADDON.setSetting('oldpass',ADDON.getSetting('pass'))
    
if not ADDON.getSetting('oldpass')==ADDON.getSetting('pass'):
    ADDON.setSetting('oldpass',ADDON.getSetting('pass'))
    utils.notification('Password and Old password are different - Please login!')
    time.sleep(2)
    LOGIN()     							### NTV.mx 3.5.0              
...
def TVGUIDE(name,cat):
	...
	for field in guide:
		...
		activerecording= recordings.getRecordingsActiveAllCat(startDate,endDate)
				...
				if (endDate < now) or ('Recursive:' in name):
					if ('Recursive:' in name):
						if utils.directprograms(cat) == '':
							...
							addDir("Recursive " + str(cat) + ': [COLOR lightgreen]' + nameRecursive + '[/COLOR]','url',200,newiconurl,cat,startDate,displaydescription,startDate,endDate,recordname)
					else:
					...
					if utils.directprograms(cat) == '':
						...
						addDir(time+'[COLOR lightgreen]'+ name + '[/COLOR]', 'url', 200, newiconurl, cat, startDate, displaydescription, startDate, endDate, recordname)
			#setView('movies', 'recordingsplanned-view')
...
def PLAY_STREAM(name,url,iconimage,cat):
	...
	if '"allown":false' in link:
		...
		if timeshiftsetting == '2':
			utils.log('default.py','RecordTimeShift(timeshift, url= %s, playpath= %s, app= %s)' % (url, playpath,app))
			timeshiftfile = ADDON.getSetting('timeshiftfile')
			timeshiftfileold = timeshiftfile.replace('.mp4','OLD.mp4')
			utils.log('default.py','shutil.copyfile(timeshiftfile= %s,timeshiftfileold= %s)' % (timeshiftfile,timeshiftfileold))
			deleteFile(timeshiftfileold)
			try:
				shutil.copyfile(timeshiftfile,timeshiftfileold)  ###
				utils.log('default.py','shutil.copyfile(timeshiftfile,timeshiftfileold) EXECUTED')
			except:
				pass
			if os.path.isfile(timeshiftfile) == True: 
				...
				while not test == True and count < 10:
					...
					count += 1
					test = os.path.isfile(timeshiftfile)
					utils.log('default.py','RecordTimeShift test delete count= %s' % repr(count))
			...
			"""
			utils.log('default.py: PLAY_STREAM','RecordTimeShift')
			...
			"""
			utils.log('default.py','RecordTimeShift 0 file= %s' % repr(timeshiftfile))
			timeshiftfile = ADDON.getSetting('timeshiftfile')
			utils.log('default.py','RecordTimeShift 0+ file= %s' % repr(timeshiftfile))
			count = 0
			while timeshiftfile == '' and count < 10:
				...
				count += 1
				timeshiftfile = ADDON.getSetting('timeshiftfile')
				utils.log('default.py','RecordTimeShift count= %s' % repr(count))
				utils.log('default.py','RecordTimeShift 0+++ file= %s' % repr(timeshiftfile))
			utils.log('default.py','RecordTimeShift count= %s' % repr(count))
			liz=xbmcgui.ListItem(playername, iconImage="DefaultVideo.png", thumbnailImage=iconimage)
			liz.setInfo( type="Video", infoLabels={ "Title": playername} )
			liz.setProperty("IsPlayable","true")
			utils.log('default.py','timeshiftfile= %s' % (timeshiftfile))
			liz.setPath(timeshiftfile)
			utils.log('default.py','RecordTimeShift 0++++ file= %s' % repr(timeshiftfile))
			...
			"""
			if os.path.isfile(timeshiftfile) == True: 
				utils.log('default.py','RecordTimeShift 1')
			...
			utils.log('default.py: PLAY_STREAM','No RecordTimeShift')
			liz=xbmcgui.ListItem(playername, iconImage="DefaultVideo.png", thumbnailImage=iconimage)
			liz.setInfo( type="Video", infoLabels={ "Title": playername} )
			liz.setProperty("IsPlayable","true")
			liz.setPath(url)
...
def RecordTimeShift(name, uri, playpath,app,cat):
	...
	nameAlarm = 'record-timeshift' + name
	try:
		if playpath == None:
			print 'No playpath'
		else:
	...
	utils.log('default.py', 'cmd= %s' % cmd)
	xbmc.executebuiltin(cmd)  # Active
	...
	for infile in files1:
		if os.path.isdir(infile):
			...
			if not infile[-4:].lower()=='.zip' and not infile[-4:].lower()=='.txt':
				name = infile.replace(recordPath,'').replace('.flv','').replace('.mp4','')
				...
				liz.addContextMenuItems(contextMenu,replaceItems=True)
				utils.log('default.py','xbmcplugin.addDirectoryItem(handle = int(sys.argv[1])= %s, url=infile= %s,listitem = liz= %s, isFolder = False)' % (repr(int(sys.argv[1])),repr(infile),repr(liz)))
...
def addDir(name,url,mode,iconimage,cat,date,description,startDate='',endDate='',recordname=''):
		...
		################ Suggested code to avoid: WARNING: CreateLoader - unsupported protocol(plugin) in plugin://plugin.video.wozboxntv64x/
		...
		if mode==200 or mode==12:
			...
			if not ADDON.getSetting('DebugRecording')=='true':
...
if mode==None or url==None or len(url)<1:
	...
	#playcmd='plugin://plugin.video.wozboxntv64x/?url=url&mode=200&name=IPTV&iconimage=http://www.ntv.mx/res/content/tv/240.png&cat=%s'% cat
	# Show menuentry 
