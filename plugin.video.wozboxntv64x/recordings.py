...
#5 USA=plugin://plugin.video.ntv/?url=url&mode=200&name=5+USA+-+%5BCOLOR+yellow%5Dn+a%5B%2FCOLOR%5D&iconimage=http%3A%2F%2Fwww.ntv.mx%2Fres%2Fcontent%2Ftv%2F64.png&cat=64
	...
	ntvchannelsm3u = os.path.join(datapath,'ntvchannels') + '.m3u'
	...
	if os.path.isfile(ntvchannelsm3u) == True: 
		utils.log('ntvchannelsm3u','ntvchannelsm3u exists - deleting')
	...
	if os.path.isfile(ftvntvini) == False:
		...
		LF.close()
	if os.path.isfile(ntvchannelsm3u) == False:
		now= datetime.datetime.today().strftime('%Y-%m-%d %H:%M:%S')
		# create file
		utils.log('ntvchannelsm3u','ntvchannelsm3u beeing created ' + now)
		LF = open(ntvchannelsm3u, 'a')
		# Write to our text file the information we have provided and then goto next line in our file.
		LF.write('#EXTM3U\n')
		IPTVCHANNELlist(LF)
		LF.write('\n')
		LF.write('# This file contains your favorite NTV channels for use with PVR IPTV Simple Client\n')
		LF.write('# Created by ' + ADDON.getAddonInfo('name') +' at ' + now + '\n')
		now= datetime.datetime.today().strftime('%Y-%m-%d %H:%M:%S')
		LF.write('# End of NTV channels - ended at ' + now + '\n')
		# Close our file so no further writing is posible.
...
def IPTVCHANNELlist(LF):
	...
	for field in channels:
		...
		line='#EXTINF:-1,' + name
		LF.write(line + '\n')
		#line='http://127.0.0.1:8080/jsonrpc?request={"jsonrpc":"2.0","method":"Addons.ExecuteAddon","params":{"wait":false,"addonid":"plugin.video.wozboxntv","params":["url=url","cat='+channel+'","mode=210","name='+name.replace(' ','%20')+'"]},"id":200}'
		line='http://127.0.0.1:8080/jsonrpc?request={"jsonrpc":"2.0","method":"Addons.ExecuteAddon","params":{"wait":false,"addonid":"' + ADDON.getAddonInfo('id') + '","params":["url=url","cat='+channel+'","mode=210","name='+name.replace(' ','%20')+'"]},"id":200}'
		LF.write(line + '\n')
...
def restoreLastSetupXml():
	...
	for infile in files1:
		...
		if firstmailoccurance:
			...
			utils.log('Storedusers',repr(storedusers))
...
def getRecordings():
	...
	utils.log('getRecordings',"len(recordings)= " + str(len(recordings)))
	for index in range(0, len(recordings)):
		cat         = recordings[index][0]
		name        = recordings[index][1]
		startDate   = parseDate(recordings[index][2])
		utils.log('getRecordings','recordings name=%s, StartDate=%s' %(repr(recordings[index][1]),repr(recordings[index][2])))
...
def schedule(cat, startDate, endDate, recordname, description):
		...
		if recordInactive:
			...
			return
		unblock = utils.directprograms(cat)
		if not unblock == '':
			...
			if not cat in getRecordingsActiveAllCat(startDate, endDate):
				...
				if not recordRescheduled:
					...
					if not recordRecursive:
						utils.notification('[COLOR orange]Direct Channel Recording: [/COLOR] %s' % (playchannel + ': ' + recordname))
