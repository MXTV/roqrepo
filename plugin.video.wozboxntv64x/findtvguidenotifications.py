...
def AUTH():
        utils.log('findtvguidenotifications.py','AUTH')
        try:
        ...
        username =ADDON.getSetting('user')
        password =md5(ADDON.getSetting('pass')).hexdigest()
        utils.log('findtvguidenotifications.py','username= %s, password= %s' % (repr(username),repr(password)))
        if not '@' in username:
        	utils.log('findtvguidenotifications.py','firstrun.Launch()')
        	import firstrun
        url = definition.getBASEURL() +'/?' + recordings.referral() + 'c=3&a=4&email=%s&psw2=%s&rmbme=on'%(username,password)
        utils.log('findtvguidenotifications.py','url= %s' % repr(url))
        html = net.http_GET(url, headers={'User-Agent' :UA}).content
        utils.log('findtvguidenotifications.py','html= %s' % repr(html))
        if 'success' in html and '@' in username:
            LOGIN()
            ADDON.setSetting('firstrun','true')
        else:
            utils.log('findtvguidenotifications.py','firstrun.Launch(1)')
            import firstrun
...
def findtvguidenotifications():
	if locking.isAnyRecordLocked():
		...
		findtvguidenotificationsend('Recording - Grab disabled')
		return
	elif  locking.isAnyScanLocked():
		findtvguidenotificationsend('Already scanning - New grab disabled')
		return
	else:
		locking.scanLock('findtvguidenotifications')
	if not locking.isScanLocked('findtvguidenotifications'):
		findtvguidenotificationsend('Scan lock not working - Grab disabled')
		return
	...
	findtvguidenotificationsend('Grab from TV Guide finished!')
						...
						for prog in range(0, len(programs)):
							...
							if cat == '0':  ## Channel from TVguide not found ##
								...
								if len(channelconvert) == 0:
									...
									utils.log('TESTfindrecursiverecordingsinGuide', 'SelectedChannel= %s' % (repr(SelectedChannel)))
									if SelectedChannel <> -1:
										try:
											...
											if int(id) > 0:
												...
												utils.log('TESTfindrecursiverecordingsinGuide', 'id= %s' % (repr(id)))
												cat = id
												utils.log('TESTfindrecursiverecordingsinGuide', 'Selected cat= %s' % (repr(cat)))
									...
									utils.log('TESTfindrecursiverecordingsinGuide', 'Converted cat= %s' % (repr(cat)))
							...
							utils.log('findrecursiverecordingsinGuide', 'cat= %s' % repr(cat))
							utils.log('findrecursiverecordingsinGuide', 'pdescription= %s' % repr(pdescription))
							if int(cat) > 0:
...
def grabGuideCode(guide,EPG_DB):		
	...
	for index in range(0, len(notifications)): 
		...
		for prog in range(0, len(programs)):
			...
			utils.log('findtvguidenotifications', 'cat= %s' % repr(cat))
			if int(cat) > 0:
				utils.log('findtvguidenotifications', 'pdescription= %s' % repr(pdescription))
