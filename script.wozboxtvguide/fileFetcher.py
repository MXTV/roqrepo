...
MAIN_URL = 'http://wozboxtv.com/downloads/xmltv/'
...
class FileFetcher(object):
    ...
    basePath = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'script.wozboxtvguide'))
    filePath = ''
    fileUrl = ''
    addon = None
    ...
    def fetchFile(self):
        ...
        if fetch:
            ...
            if self.fileType == self.TYPE_REMOTE:
                xbmc.log('[script.wozboxtvguide] file is in remote location: %s' % self.fileUrl, xbmc.LOGDEBUG)
                if not xbmcvfs.copy(self.fileUrl, tmpFile):
                    xbmc.log('[script.wozboxtvguide] Remote file couldn\'t be copied: %s' % self.fileUrl, xbmc.LOGERROR)
            else:
                f = open(tmpFile, 'wb')
                xbmc.log('[script.wozboxtvguide] file is on the internet: %s' % self.fileUrl, xbmc.LOGDEBUG)
            ...
            if os.path.getsize(tmpFile) > 256:
                ...
                xbmc.log('[script.wozboxtvguide] file %s was downloaded' % self.filePath, xbmc.LOGDEBUG)
            else:
