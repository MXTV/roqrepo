...
class TVGuide(xbmcgui.WindowXML):
    ...
    def onActionEPGMode(self, action):
        ...
        if action.getId() == ACTION_LEFT:
            ...
            xbmc.log('[script.wozboxtvguide] Unhandled ActionId: ' + str(action.getId()), xbmc.LOGDEBUG)
    ...
    def _showContextMenu(self, program):
        ...
        if buttonClicked == PopupMenu.C_POPUP_REMIND:
            ...
            if selection == 0:
                xbmc.executebuiltin("RunPlugin(plugin://plugin.video.wozboxlink/movies/play_by_name/%s/%s)" % (
                    title, program.language))
            elif selection == 1:
                if program.season and program.episode:
                    xbmc.executebuiltin("RunPlugin(plugin://plugin.video.wozboxlink/tv/play_by_name/%s/%s/%s/%s)" % (
                        title, program.season, program.episode, program.language))
                else:
                    xbmc.executebuiltin("RunPlugin(plugin://plugin.video.wozboxlink/tv/play_by_name_only/%s/%s)" % (
                        title, program.language))
    ...
    def playChannel(self, channel, program = None):
        ...
        if url:
            if str.startswith(url,"plugin://plugin.video.wozboxlink") and program is not None:
                import urllib
...
class StreamSetupDialog(xbmcgui.WindowXMLDialog):
    ...
    def updateAddonInfo(self):
        ...
        for (label, stream) in streams:
            if item.getProperty('addon_id') == "plugin.video.wozboxlink":
